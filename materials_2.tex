...
Equation~\ref{eq:SIM_illumination_invFT} shows (XX maybe you mean
soemthing else? XX) there is clearly only
...
which increases with depth (XX many other factors which prevent
infinite depth imaging, absorption, working distance of lens etc... XX). Nonetheless, by correcting for
...
\textit{Drosophila melanogaster} neuro-muscular junction (NMJ)
sample. (XX should stress this is a live sample XX)
...
green and red channels respectively is $291$nm and $338$nm (XX say
using the rayleigh criterion with emission at 525 and 640 nm or
whatever you used XX). The stripe 
...
This structure was chosen for two principal reasons. Firstly, the known
...
is slightly better than the anticipated lateral resolution (XX marked
on figure by arrows XX). This is not 
...
makes separations $+150$nm (XX dont know what you mean by +150 nm XX) unresolvable in parts. The $z$ length of the 
...
\begin{figure}
	...
	\caption[Argolight slide data]{Argolight slide data. Both \textbf{(a)} without AO correction and \textbf{(b)} with AO correction datasets 
		...
		a $xz$ slice at the midpoint of the $y$-axis
                (bottom). Without AO correction data is displayed with
                the maximum intensity level of data with AO correction
                for clarity. The spherical aberration mismatch from
                the SIMCheck ImageJ plug-in\cite{ball2015simcheck} is
                shown \textbf{(c)} without AO correction and
                \textbf{(d)} with AO correction (XX could you mark the
        relevant line pair either with an arrow or with 150 nm above
        it in both images to make comparison easier XX)}
...
are observed in both imaging channels. Much of the membrane
labelled-HRP (XX no the HRP is labelling the membrane not the other
way round XX) is 
not visible without AO correction. Almost all of the BRP (XX you dont
seem to define this anywhere, what is it? how is it labelled
etc... XX) is extremely dim 
...
\begin{figure*}
	...
	data acquired on the DeepSIM imaging system]{Maximum intensity 
		...
		samples acquired without (left) and with (right) AO
                correction. GFP (XX This is a dye what is it labelling XX) 
		shown in yellow, HRP (XX this is a protein, what is it
                labelled with, and what structure is it showing? XX) shown in magenta. The arrows highlight 
...
\begin{figure*}
	...
	data acquired on the DeepSIM imaging system]{Fourier plots of 
		...
		correction \textbf{(b)} Alexa568 (XX previous fig said
                HRP, but probbaly what you really want is what is it
                labelling XX) channel without AO correction 
