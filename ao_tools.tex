...
Section~\ref{subsec:zernike_mode_measurment}, an equal (XX surely this
is 1/2 the error as the mirror effectively gives you 2x the applied
phase correction XX) and opposite magnitude 
...
fitting has bounds defined as $(j_{max}-j_{min}) \pm
0.25(j_{max}-j_{min})$. (XX this doesnt seem right, as this means the
result must be in the central 50\% of the input range, is this really
how you run it or is this formula wrong? XX)
...
the spinning disk (XX not sure what you are trying to say but this
isnt a senstence XX) . If follow that the image quality of both systems is
