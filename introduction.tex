...
arguably more profound since previous fluorphores were synthetic molecules 
...
of interest to which the fluorphores are conjugated. For example, a 
fluorphore with highly non-specific binding may be imaged by a microsope with 
...
Consider a point source located at $\textbf{O}$ in a media of refractive 
...
techniques. Table~\ref{tab:technique_res} shows the typical lateral and axial resolution limits of the conventional and far-field super-resolution techniques, aswell as whether these resolutions are primarily affected by the numerical aperature of the imaging system\cite{schermelleh2010guide,schermelleh2019super,von2017three,blom2014sted,vicidomini2018sted,gustafsson2000surpassing}:
...
\begin{table}[H]
	...
	\end{tabular}
	\caption{\label{tab:technique_res}Typical lateral and axial resolutions of the imaging technique categories and whether those resolutions are affected by numerical aperature.}
\end{table}

Each super-resolution technique has benefits and drawbacks which 
...
where $\textbf{r}'$ denotes the sample reference frame, $\textbf{r}$ denotes
...
generally arise in a biological context. Additionally, the Zernike modes 
have a number of useful mathematical properties, and low order Zernike 
modes correspond to well-known optical aberrations e.g. defocus, 
